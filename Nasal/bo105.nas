...
controls.flapsDown = func(v) {
	if (!flap_mode) {
		...
		if (flap_mode == 1) {
			if (elapsedN.getValue() < down_time + 0.05)
				return;
...
dynamic_view.register(func {
	...
	me.heading_offset =						# heading change due to
		(me.roll < 0 ? -5 : -5) * r * abs(r);			#    roll left/right

	me.pitch_offset =						# pitch change due to
		(me.pitch < 0 ? -5 : -3) * sin(me.pitch) * lowspeed	#    pitch down/up
		+ 10 * sin(me.roll) * sin(me.roll);			#    roll
...
aircraft.livery.init("Aircraft/bo105/Models/Liveries");
